...
def get_gripper_width(img_list):
    ...
    # 进行高斯平滑处理
    smoothed_distances = gaussian_filter1d(new_distances, sigma=2)
...
def normalize_ik_and_save_hdf5(args):
    ...
    with h5py.File(input_file, "r") as f_in:
        ...
        for i in range(len(data)):
            ...
            tcp_qpos = np.array([direction[0], direction[1], direction[2], q[0], q[1], q[2], q[3]])
            tcp_qpos_list.append(tcp_qpos)

        tcp_qpos_list = np.array(tcp_qpos_list)

        for i in range(gripper_width.shape[0]):
            if gripper_width[i] < config["gripper_threshold"]:
                gripper_width[i] = 0
            else:
                gripper_width[i] = 1

        new_tcp_qpos_list = np.concatenate((tcp_qpos_list, gripper_width), axis=1)

        with h5py.File(output_file, "w") as f_out:
            f_out.create_dataset("action", data=new_tcp_qpos_list)
            ...
            observations_group.create_dataset("qpos", data=new_tcp_qpos_list)
