    ...
    try:
        with h5py.File(input_file, "r") as f_in:
            ...
            with h5py.File(output_file, "w") as f_out:
                ...
                cam_hight, cam_width = (
                    f_in["observations/images/front"].shape[1],
                    f_in["observations/images/front"].shape[2],
                )
...
if __name__ == "__main__":
    ...
    file_list = [filename for filename in os.listdir(input_dir) if filename.endswith(".hdf5")]
    args_list = []
    ...
    with Pool(num_processes) as pool:
        list(
            tqdm(
                pool.imap_unordered(normalize_and_save_base_tcp_hdf5, args_list),
                total=len(args_list),
