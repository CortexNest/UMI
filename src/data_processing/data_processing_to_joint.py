...
START_QPOS = config[
    "start_qpos"
]  # Initial joint positions for the robot (values specific to your robot's configuration)
PI = np.pi

# Load the robot chain
my_chain = ikpy.chain.Chain.from_urdf_file(
    config["urdf_path"], base_elements=["world"]
)  # Path to the URDF file of the robot model (replace with your robot's URDF file in config.json)
...
def cartesian_to_joints(position, quaternion, initial_joint_angles=None, **kwargs):
    ...
    joint_angles = my_chain.inverse_kinematics(
        position, rotation_matrix, orientation_mode="all", initial_position=initial_joint_angles
    )
...
def normalize_ik_and_save_hdf5(args):
    ...
    )  # Initial position of the robot's base in 3D space (in meters)
    ...
    )  # Initial orientation of the robot's base in 3D space (in roll, pitch, yaw format) (in degrees)
